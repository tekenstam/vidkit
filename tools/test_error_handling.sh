...
echo -e "\n--- Test 1: Non-existent file ---"
./vidkit --preview -b test_results/errors/nonexistent_file.mp4 2>&1 | tee output.log
...
./vidkit --preview -b test_results/errors/input/invalid.mp4 2>&1 | tee output.log
...
if [ -d "test_videos" ] && [ -n "$(ls -A test_videos/*.mp4 2>/dev/null)" ]; then
  ...
  ./vidkit --preview -b --movie-dir "test_results/errors/readonly" "$TEST_VIDEO" 2>&1 | tee output.log
...
if [ -d "test_videos" ] && [ -n "$(ls -A test_videos/*.mp4 2>/dev/null)" ]; then
  # Find a test video
  TEST_VIDEO=$(ls test_videos/*.mp4 | head -1)
  ./vidkit --preview -b --movie-provider invalid_provider "$TEST_VIDEO" 2>&1 | tee output.log
...
if [ -d "test_videos" ] && [ -n "$(ls -A test_videos/*.mp4 2>/dev/null)" ]; then
  ...
  ./vidkit --preview -b --movie-provider tmdb --no-metadata=false "$TEST_MOVIE" 2>&1 | tee output.log
...
if [ -d "test_videos" ] && [ -n "$(ls -A test_videos/*.mp4 2>/dev/null)" ]; then
  # Find a test video
  TEST_VIDEO=$(ls test_videos/*.mp4 | head -1)
  ./vidkit --preview -b --movie-format "{invalid_variable}" "$TEST_VIDEO" 2>&1 | tee output.log
...
./vidkit --preview -b "$SPECIAL_CHAR_FILE" 2>&1 | tee output.log
if grep -q "panic\|crash" output.log; then
...
./vidkit --preview -b "$LONG_NAME_FILE" 2>&1 | tee output.log
if grep -q "panic\|crash" output.log; then
