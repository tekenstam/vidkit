...
echo -e "\n--- Test 4: No-metadata flag ---"
./vidkit --preview --batch --no-metadata "$TEST_MOVIE" | tee output.log
...
echo -e "\n--- Test 5: Language option ---"
./vidkit --preview --batch --lang fr "$TEST_MOVIE" | tee output.log
...
./vidkit --preview --batch --movie-format "$CUSTOM_FORMAT" "$TEST_MOVIE" | tee output.log
...
echo -e "\n--- Test 7: Lowercase flag ---"
./vidkit --preview --batch --lowercase "$TEST_MOVIE" | tee output.log
...
./vidkit --preview --batch -s "$TEST_MOVIE" | tee output.log
...
echo -e "\n--- Test 10: Provider selection ---"
./vidkit --preview --batch --movie-provider tmdb "$TEST_MOVIE" | tee output.log
...
./vidkit --preview --batch --movie-dir "$DIR_TEMPLATE" "$TEST_MOVIE" | tee output.log
...
echo -e "\n--- Test 13: Unknown flag handling ---"
./vidkit --preview --batch --non-existent-flag "$TEST_MOVIE" 2>&1 | tee output.log
