...
./vidkit --preview $NO_METADATA --movie-dir "test_results/Movies/{title} ({year})" test_videos/The.Shawshank.Redemption.1994.1080p.BluRay.x264.mp4

# Test alphabetical organization
echo -e "\n--- Alphabetical Movie Organization ---"
./vidkit --preview $NO_METADATA --movie-dir "test_results/Movies/{title[0]}/{title} ({year})" test_videos/The.Shawshank.Redemption.1994.1080p.BluRay.x264.mp4

# Test genre-based organization
echo -e "\n--- Genre-Based Movie Organization ---"
./vidkit --preview $NO_METADATA --movie-dir "test_results/Movies/By Genre/{genre}/{title} ({year})" test_videos/The.Shawshank.Redemption.1994.1080p.BluRay.x264.mp4

echo -e "\n=== Testing TV Show Organization ===\n"

# Test basic TV show organization
echo -e "\n--- Basic TV Show Organization ---"
./vidkit --preview $NO_METADATA --tv-dir "test_results/TV Shows/{title}/Season {season:02d}" test_videos/Breaking.Bad.S01E01.Pilot.mp4

# Test network-based organization
echo -e "\n--- Network-Based TV Show Organization ---"
./vidkit --preview $NO_METADATA --tv-dir "test_results/TV Shows/{network}/{title}/Season {season}" test_videos/Breaking.Bad.S01E01.Pilot.mp4

# Test year-based organization
echo -e "\n--- Year-Based TV Show Organization ---"
./vidkit --preview $NO_METADATA --tv-dir "test_results/TV Shows/By Year/{year}/{title}/Season {season:02d}" test_videos/Breaking.Bad.S01E01.Pilot.mp4
...
if [ -z "$NO_METADATA" ]; then
  ...
  echo -e "\n=== Skipping Provider-Specific Tests (No Metadata Mode) ===\n"
fi
